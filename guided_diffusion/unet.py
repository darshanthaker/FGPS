...
class UNetModel(nn.Module):
    ...
    def get_attention(self, module):
        for m in module:
            if isinstance(m, AttentionBlock):
                weight = m.attention.weight
                weight = weight.mean([0, 1]).reshape((1, 1, int(weight.shape[-1]**0.5), int(weight.shape[-1]**0.5)))
                weight = F.interpolate(
                    weight, size=(256, 256), mode='nearest'
                ).squeeze()
                return weight.cpu().detach().numpy()
        return None
