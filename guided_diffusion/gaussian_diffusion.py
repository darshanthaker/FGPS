...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        pbar = tqdm(list(range(self.num_timesteps))[::-1],disable=False)
        for idx in pbar:
            cur_dist = torch.tensor(float("inf"), dtype=torch.float)
            time = torch.tensor([idx] * img.shape[0], device=device)
            img = img.requires_grad_()
            out = self.p_sample(x=img, t=time, model=model, label=label)
            xt_k = out["sample"]
            x_prev = img
            x_0_hat = out['pred_xstart']
                
            noisy_measurement = self.q_sample(measurement, t=time)
            xt_k, distance = measurement_cond_fn(x_t=xt_k,
    ...
    def p_sample(self, model, x, t, label):
        raise NotImplementedError

    def p_mean_variance(self, model, x, t, label):
        model_output = model(x, self._scale_timesteps(t), y=label)
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def p_mean_variance(
        self, model, label, *args, **kwargs
    ):  # pylint: disable=signature-differs
        return super().p_mean_variance(self._wrap_model(model), label, *args, **kwargs)
...
class DDPM(SpacedDiffusion):
    def p_sample(self, model, x, t, label):
        out = self.p_mean_variance(model, x, t, label)
        sample = out['mean']
...
class DDIM(SpacedDiffusion):
    def p_sample(self, model, x, t, eta=0.0, label=None):
        out = self.p_mean_variance(model, x, t, label)
